...
### Перенос строк
Если у вас в коде имеется длинная строка, которую вы хотите перенести на другую строчку, чтобы визуально это смотрелось красиво и удобно, можно сделать следующее:
```bat
@echo off
chcp 65001
...
set "n=&echo."
echo Это можно сказать простая табуляция.%n%Точнее обычная переменная, которая может спокойно перенести строчку на следующую.%n%^
